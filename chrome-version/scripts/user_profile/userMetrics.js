export function userMetrics() {
  ...
  // *** Volet "Statistiques"
    ...
    const observer = new MutationObserver(() => {
      ...
      if (messageElement) { // RESTE : À SUPPRIMER LA BALISE FONT ET "Message(s)"
        const messages = document.querySelector("a.actualites_side_item > font");

        if (messages) { // Delete the number of messages
          ...
          messages.remove();
          messageElement.innerHTML = messageElement.innerHTML.replace('Message(s)', message);
      ...
      if (titres) {
        ...
        Array.from(titres).forEach((titre) => {
          if (titre.textContent.includes("Contributions appréciées") ||titre.textContent.includes("Notez vos lectures")) {
            titre.remove()
          }
        });
      }
